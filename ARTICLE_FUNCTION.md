...
9. **Pinterest CSV — только по явному запросу:** обычная публикация статьи не
   ...
   прямой Etsy-link требует отдельного явного указания. CSV обязан следовать
...
- **Google Drive source assets are READ/COPY ONLY (2026-07-15):** никогда не
  удалять, не переносить, не переименовывать, не перезаписывать и не
  "наводить порядок" в `/Users/kseniateter/My Drive/Sentimentalica/` или
  папках листингов. Страницы листингов, thumbnails, refs, customer/export
  картинки — source of truth. Агент может только читать/смотреть их и копировать
  производные в `staging/` или `public/blog/img/`. Если файл в Drive выглядит
  неправильным, дублируется, пропал или лежит не там — остановиться и спросить
  Ксению; не исправлять самостоятельно.
