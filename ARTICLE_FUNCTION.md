...
- **Слот gen2 = мокап**: в prompts JSON помечать `"type":"mockup",
  ...
  — до **3 НАСТОЯЩИХ страниц** из customer-папки (asset-тул кладёт их как pageN.jpg) — SD генерит стол сверху с
  пустым центром, а НАСТОЯЩАЯ страница листинга компонуется туда как напечатанный
  лист (кромка печати, тень, лёгкий поворот). Минимум один мокап на статью.
