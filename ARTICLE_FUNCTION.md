...
- **Референсы Ксении = ФАЙЛЫ, не мои слова (2026-07-07):** папки `refs/scenes/`,
  `refs/infographics/`, `refs/50things/` — эталон. Критик и генератор сверяются
  с картинками ОТТУДА, а не с моим пересказом в SCENE_STYLE.md. Пустая папка =
  агент слепой. Сцена обязана дотягивать до `refs/scenes/` (атмосфера-эскейп,
  «gen3»-планка), инфографика — копировать `refs/infographics/` 1:1.
...
- **Листикл → инфографика обязательна:** каждая статья-список получает полную
  инфографику (`render_list_infographic.py`, SD-фон в палитре темы + рваные
  панели) — в начало статьи, обложкой и пином.
